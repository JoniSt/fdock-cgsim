...
//The function performs the docking algorithm with the CPU.
{
	...
	*cpu_exectime_sum = 0;
	...
	for (run_cnt = 0; run_cnt < mypars->num_of_runs; run_cnt++)
	{
		...
		get_seeds_and_initpop(mypars, init_population, &myligand, mygrid);	//Capturing seeds and initial population

		get_ref_orientation(&myligand, mypars);	//Capturing reference orientation of the ligand

		for (i=0; i<CPU_MAX_POP_SIZE; i++)
			for (j=0; j<3; j++)
				init_population [i][j] = init_population [i][j]/mygrid->spacing;		//x, y and z in grid spacing instead of A
		...
		//Unit and moving vectors of the ligand's rotatable bonds must be re-calculated due to the new orientation and scaling
		...
		printf("Run %d started...     ", run_cnt+1);
		fflush(stdout);
