...
int get_gridinfo(const char* fldfilename, Gridinfo* mygrid)
{
	...
	{
		char* filename_copy = strdup(fldfilename);
		mygrid->grid_file_path = strdup(dirname(filename_copy));
		free(filename_copy);
	}
...
//the return value is 0.
{
	...
	*fgrids = (double*) malloc((sizeof(double))*(mygrid->num_of_atypes+2)*(mygrid->size_xyz [0])*
	...
	for (t=0; t<mygrid->num_of_atypes+2; t++)
	{
		...
		//Added by L30nardoSV (04/2023)
		//Adding the full path of the associated grid files, otherwise the program doesn't find them
		//Alternatively the binary must be copied into the folder containing all map files
